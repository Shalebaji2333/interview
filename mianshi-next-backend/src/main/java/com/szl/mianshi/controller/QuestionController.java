...
import com.szl.mianshi.common.DeleteRequest;
...
import com.szl.mianshi.manager.CounterManager;
import com.szl.mianshi.model.dto.question.*;
...
import com.szl.mianshi.sentinel.SentinelConstant;
...
public class QuestionController {
    ...
    @PostMapping("/thumb")
    public BaseResponse<Boolean> thumbQuestion(@RequestParam long questionId, HttpServletRequest request) {
        if (questionId <= 0) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR);
        }
        // 登录才能点赞
        ...
        ThrowUtils.throwIf(question == null, ErrorCode.NOT_FOUND_ERROR);
        // 是否已点赞
        long result = questionService.doQuestionThumb(userId, questionId);
        return ResultUtils.success(result > 0);
    }

    @GetMapping("/thumb/check")
    public BaseResponse<Boolean> checkQuestionThumb(@RequestParam long questionId, HttpServletRequest request) {
        if (questionId <= 0) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR);
        }
        // 登录才能点赞
        ...
        ThrowUtils.throwIf(question == null, ErrorCode.NOT_FOUND_ERROR);
        // 获取点赞状态
        boolean result = questionService.hasQuestionThumb(userId, questionId);
        return ResultUtils.success(result);
    }

    @GetMapping("/thumb/count")
    public BaseResponse<Long> getQuestionThumbCount(@RequestParam long questionId) {
        if (questionId <= 0) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR);
        }
        // 获取点赞数
        long result = questionService.getQuestionThumbCount(questionId);
        return ResultUtils.success(result);
    }
}
