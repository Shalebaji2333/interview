...
import com.szl.mianshi.mapper.QuestionMapper;
...
import com.szl.mianshi.model.entity.QuestionBankQuestion;
...
import com.szl.mianshi.service.QuestionBankQuestionService;
import com.szl.mianshi.service.QuestionService;
import com.szl.mianshi.service.UserService;
import com.szl.mianshi.service.QuestionThumbService;
import com.szl.mianshi.utils.SqlUtils;
...
public class QuestionServiceImpl extends ServiceImpl<QuestionMapper, Question> implements QuestionService {
    ...
    @Override
    public long doQuestionThumb(long userId, long questionId) {
        return this.doQuestionThumbInner(userId, questionId);
    }
    ...
    public int doQuestionThumbInner(long userId, long questionId) {
        ...
        QuestionThumb oldQuestionThumb = questionThumbService.selectOne(thumbQueryWrapper);
        boolean result;
        // 已点赞
        if (oldQuestionThumb != null) {
            result = questionThumbService.deleteById(oldQuestionThumb.getId()) > 0;
            return result ? -1 : 0;
        } else {
            result = questionThumbService.insert(questionThumb) > 0;
            return result ? 1 : 0;
        }
    }

    @Override
    public boolean hasQuestionThumb(long userId, long questionId) {
        QuestionThumb questionThumb = new QuestionThumb();
        questionThumb.setUserId(userId);
        questionThumb.setQuestionId(questionId);
        QueryWrapper<QuestionThumb> thumbQueryWrapper = new QueryWrapper<>(questionThumb);
        return questionThumbService.selectCount(thumbQueryWrapper) > 0;
    }

    @Override
    public long getQuestionThumbCount(long questionId) {
        QuestionThumb questionThumb = new QuestionThumb();
        questionThumb.setQuestionId(questionId);
        QueryWrapper<QuestionThumb> thumbQueryWrapper = new QueryWrapper<>(questionThumb);
        return questionThumbService.selectCount(thumbQueryWrapper);
    }
